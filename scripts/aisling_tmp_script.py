...
def add_automated_camera_info(engine, from_schema: str, from_table: str, target_schema: str, target_table: str):
    ...
    records = [loc for (loc,) in engine.execute("select distinct location from tmp.moving_violations_need_geo where geography is null limit 5000").fetchall()]
...
def generate_moving_violations_table (engine, **kwargs):
    next_tables=add_automated_camera_info(engine=engine, target_schema='tmp', target_table='moving_violations_automated_cameras', from_schema='tmp', from_table='moving_violations_has_geo')
