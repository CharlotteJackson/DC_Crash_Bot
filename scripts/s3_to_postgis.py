...
def s3_to_postGIS (folder_to_load:str, AWS_Credentials:dict, format:str, header:str, mode:str,move_after_loading:str,move_to_folder:str):
    ...
    files_to_load = [(urllib.parse.quote(obj.key), obj.key, obj.Object().metadata['target_schema'],obj.Object().metadata['target_table']) for obj in bucket.objects.filter(Prefix=folder_to_load, Delimiter='/') if 'target_table' in obj.Object().metadata.keys() if format in obj.key]
